...
from src.quantum.quantum_solvers import solve_xy
...
def main():
    config = load_config()
    p_list = config['solvers']['xy_qaoa']['p']
    maxiter = config['solvers']['xy_qaoa']['maxiter']
    shots = config['solvers']['xy_qaoa']['shots']
    ...
    print(f"Running XY-QAOA (maxiter={maxiter}, shots={shots}) on validation and principal instances (N <= 16)...")
    
    count = 0
    for dataset, N, instance_id, filepath in sorted_instances:
        # XY-QAOA runs on validation (N<=10) and principal (N<=16)
        if N > 16:
            continue
            
        print(f"Solving instance {dataset} (N={N}, ID={instance_id}) with XY-QAOA...")
        instance = load_instance(filepath)
        
        # Run XY-QAOA for each depth p in configuration
        for p in p_list:
            print(f"  -> Depth p = {p}")
            result = solve_xy(instance, p=p, maxiter=maxiter, shots=shots)
    ...
    print(f"XY-QAOA experiments completed for {count} executions. Results appended to {csv_path}.")
