...
from src.utils.utils import load_instance, append_results_to_csv
from src.solvers.classic_solvers import solve_exact
...
def main():
    ...
    print("Running ExactSolver (brute force) on instances with N <= 16...")
    
    count = 0
    for dataset, N, instance_id, filepath in sorted_instances:
        # Skip scalability instances for ExactSolver as N >= 18 is computationally intensive
        if N > 16:
            continue
    ...
    print(f"ExactSolver experiments completed for {count} instances. Results appended to {csv_path}.")
