...
from src.quantum.quantum_solvers import solve_qaoa
...
def main():
    config = load_config()
    p_list = config['solvers']['qaoa']['p']
    maxiter = config['solvers']['qaoa']['maxiter']
    shots = config['solvers']['qaoa']['shots']
    ...
    print(f"Running Standard QAOA (maxiter={maxiter}, shots={shots}) on validation and principal instances (N <= 16)...")
    
    count = 0
    for dataset, N, instance_id, filepath in sorted_instances:
        # QAOA runs on validation (N<=10) and principal (N<=16)
        if N > 16:
            continue
            
        print(f"Solving instance {dataset} (N={N}, ID={instance_id}) with QAOA...")
        instance = load_instance(filepath)
        
        # Run QAOA for each depth p in configuration
        for p in p_list:
            print(f"  -> Depth p = {p}")
            result = solve_qaoa(instance, p=p, maxiter=maxiter, shots=shots)
    ...
    print(f"Standard QAOA experiments completed for {count} executions. Results appended to {csv_path}.")
