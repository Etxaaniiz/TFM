...
En lugar de inicializar los parámetros del circuito $(\gamma, \beta)$ de forma aleatoria, este trabajo emplea el esquema de inicialización \textit{Trotterized Quantum Annealing} (TQA) propuesto por Sack y Serbyn \cite{SackSerbyn2021}, que emula un proceso de recocido cuántico adiabático mediante una discretización lineal de la rampa temporal de evolución y evita, según muestran los autores, los mínimos falsos del paisaje de optimización en un amplio rango de pasos temporales de Trotter. Concretamente, para una profundidad de circuito $p$, los ángulos iniciales de cada capa $m = 1, \dots, p$ se calculan como:
\begin{equation}
