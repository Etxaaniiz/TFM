...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{T3.4. Integración de solvers clásicos de referencia:} Implementación de la interfaz con Gurobi Optimizer (\textit{Branch-and-Bound}) y Simulated Annealing (\texttt{dwave-neal}), y creación de la suite de tests unitarios (\texttt{tests/}).
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{T5.2. Generación automatizada de gráficos vectoriales y tablas:} Desarrollo de scripts en Python (\texttt{matplotlib}/\texttt{seaborn}) para la renderización de figuras analíticas en alta resolución (300 DPI) y cuadros comparativos.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{2.2cm}p{7.8cm}r}
        ...
        & T3.4. Integración de solvers clásicos (Gurobi, SA) y tests unitarios & 10 \\
...
\begin{itemize}
    \item \textbf{Amortización del Hardware de Desarrollo:} Computada sobre la estación 
          de trabajo principal (procesador multinúcleo de alta frecuencia, 32~GB RAM y 
          almacenamiento NVMe valorada en 1.500~\euro), asumiendo un periodo de vida útil 
          de 36 meses y una imputación proporcional a los 6 meses de ejecución del TFM:
          \begin{equation}
              \text{Amortización HW} = \frac{1.500\text{ \euro}}{36\text{ meses}} \times 5\text{ meses} \approx 208{,}33\text{ \euro}
          \end{equation}
    \item \textbf{Consumo Eléctrico de Simulación Intensiva:} Estimación del gasto 
    ...
    \item \textbf{Herramientas de Versionado y Repositorio:} Uso de planes estándar de 
          ...
          (cobertura bajo nivel académico/abierto).
\end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.2cm}p{6.2cm}p{3.3cm}r}
        ...
        \textbf{Equipamiento Local} & Amortización estación de trabajo (Intel i7, 32 GB RAM) & 5 meses (vida útil 36 m.) & 208 \\
